...
namespace QuanLyThongTinKhachHangSacomBank.Controllers
{
    class DepositController
    {
            ...
            try
            {
                if (activeUC is IDepositViewData depositViewData)
                {
                    ...
                    if (formOTP.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        SaveTransaction(depositViewData);
        ...
        private void SaveTransaction(IDepositViewData depositViewData)
        {
            using (var connection = dbContext.GetConnection())
            {
                if (connection == null)
        ...
        private void ExportToPDF()
        {
            try
            {
                ...
                string formattedAmount = decimal.Parse(lastDepositViewData.Amount).ToString("#,##0") + " VND";
                    ...
                    if (saveFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        try
                        {
                            ...
                            using (FileStream fs = new FileStream(pdfPath, FileMode.Create, FileAccess.Write, FileShare.None))
                            {
                                ...
                                subTableTransaction.AddCell(new PdfPCell(new Phrase(lastDepositViewData.TransactionDescription, boldFont)) { HorizontalAlignment = Element.ALIGN_LEFT, Padding = 2f });
